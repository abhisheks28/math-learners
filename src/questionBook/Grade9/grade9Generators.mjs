...
export const generateRealNumbers = () => {
    // Identify irrational or Laws of Exponents
    const type = Math.random() > 0.5 ? "Irrational" : "Exponents";
    let question, answer;

    if (type === "Irrational") {
        // Simple identification: Which is irrational?
        // Options: sqrt(perfect), sqrt(non-perfect), terminating decimal, integer
        ...
        const options = shuffleArray([
            ...
            { value: `√${ps}`, label: `√${ps}` }, // Rational
            { value: `${getRandomInt(2, 10)}`, label: `${getRandomInt(2, 10)}` }, // Integer
            { value: `${getRandomInt(1, 9)}.${getRandomInt(1, 9)}`, label: `${getRandomInt(1, 9)}.${getRandomInt(1, 9)}` } // Terminating decimal
        ]);
...
export const generatePolynomialFactorization = () => {
    // Splitting middle term: x^2 + (a+b)x + ab = (x+a)(x+b)
    const a = getRandomInt(1, 5);
    const b = getRandomInt(1, 5);
...
export const generatePolynomialZeroes = () => {
    // Zero of linear polynomial ax + b
    const a = getRandomInt(2, 5);
    const b = a * getRandomInt(1, 5); // Ensure integer root
...
export const generateLinearEquationSolutions = () => {
    // Check if (x,y) is a solution to ax + by = c
    const a = getRandomInt(1, 5);
    const b = getRandomInt(1, 5);
...
export const generateLinearEquationSolving = () => {
    // Simple substitution: Find y if x=k in ax + by = c
    const a = getRandomInt(1, 5);
    const b = getRandomInt(1, 5);
...
export const generateCoordinateBasics = () => {
    // Quadrants or Plotting
    const x = getRandomInt(-5, 5);
    const y = getRandomInt(-5, 5);
    if (x === 0 || y === 0) return generateCoordinateBasics(); // Avoid axes
...
export const generateCoordinateFormulas = () => {
    // Distance Formula: dist((x1,y1), (x2,y2))
    // Use Pythagorean triplets for integer distance
...
export const generateMensurationArea = () => {
    // Heron's Formula (simple) or Area of Triangle
...
export const generateMensurationVolume = () => {
    // Surface Area or Volume of Cube/Cuboid
    ...
    if (type === "Cube") {
        ...
        question = `Find the total surface area of a cuboid with dimensions ${l}cm x ${w}cm x ${h}cm.`;
        answer = `${sa} cm²`;
    }
...
export const generateStatistics = () => {
    ...
    if (type === "Mean") {
        ...
        // Adjust last number to make mean integer
...
export const generateProbability = () => {
    // Simple probability: Coin or Dice
    ...
    if (type === "Coin") {
        ...
        // HH, HT, TH, TT -> HT, TH are favorable (2/4 = 1/2)
        answer = "1/2";
    } else {
        // Dice
    ...
    // Hardcoded fallbacks for probability
